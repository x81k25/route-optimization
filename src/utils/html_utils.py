"""
HTML utilities for generating interactive tables, reports, and route visualizations.
Consolidates HTML generation functionality from visualization.py.
"""
...
def create_comprehensive_zones_map(itinerary: pl.DataFrame, locations_path: str = "data/locations.jsonl") -> str:
    """
    Create a simple map showing all zone locations as colored points.
    
    Args:
        itinerary: DataFrame containing route optimization results for all zones
        locations_path: Path to locations file
        
    Returns:
        HTML string containing the Plotly map
    """
    # Load locations data
    locations = {}
    try:
        ...
        return "<p>Map unavailable: Locations data not found</p>"
    ...
    # Collect all coordinates to calculate center and zoom
    all_lats = []
    all_lons = []
    ...
    for zone_id in zones:
        ...
        for row in zone_data.iter_rows(named=True):
            ...
            for pos_location in pos_locations:
                all_lats.append(pos_location[1])  # latitude
                all_lons.append(pos_location[0])  # longitude
    ...
    if all_lats and all_lons:
        ...
        if max_range > 15:      # Very wide area (multiple states)
            zoom = 4
        elif max_range > 8:     # Wide area (state level)
            zoom = 5
        elif max_range > 4:     # Large area (multiple counties)
            zoom = 6
        elif max_range > 2:     # Medium area (county level)
            zoom = 7
        elif max_range > 1:     # Smaller area (city region)
            zoom = 8
        elif max_range > 0.5:   # City level
            zoom = 9
        else:                   # Local area
            zoom = 10
    else:
        # Fallback to San Francisco if no coordinates found
    ...
    # Collect all unique locations per zone
    zone_locations = {}
    
    for zone_id in zones:
        zone_data = itinerary.filter(pl.col('zone_id') == zone_id)
        zone_locations[zone_id] = {'primary': [], 'secondary': []}
        ...
        for row in zone_data.iter_rows(named=True):
            ...
            for pos_id, pos_location, pos_class in zip(pos_ids, pos_locations, pos_classes):
                if pos_id not in seen_locations:
                    seen_locations.add(pos_id)
                    lat, lon = pos_location[1], pos_location[0]  # lat, lon from coordinate pair
                    
                    # Get location name
                    name = locations[pos_id]['name'] if pos_id in locations else f"Location {pos_id}"
                    
                    zone_locations[zone_id][pos_class].append({
                        'pos_id': pos_id,
                        'name': name,
                        'lat': lat,
                        'lon': lon
    ...
    for zone_id in zones:
        ...
        primary_locs = zone_locations[zone_id]['primary']
        if primary_locs:
            fig.add_trace(go.Scattermapbox(
                lat=[loc['lat'] for loc in primary_locs],
                lon=[loc['lon'] for loc in primary_locs],
                mode='markers',
                marker=dict(
                    size=16,
                    color=zone_color,
                ...
                text=[f"{zone_id} - {loc['name']} (PRIMARY)" for loc in primary_locs],
        ...
        secondary_locs = zone_locations[zone_id]['secondary']
        if secondary_locs:
            fig.add_trace(go.Scattermapbox(
                lat=[loc['lat'] for loc in secondary_locs],
                lon=[loc['lon'] for loc in secondary_locs],
                mode='markers',
                marker=dict(
                    size=8,
                    color=zone_color
                ),
                text=[f"{zone_id} - {loc['name']}" for loc in secondary_locs],
    ...
    # Convert to HTML
    return fig.to_html(include_plotlyjs='cdn', div_id="zones-map")


def create_aggregate_report_html(aggregate_df: pl.DataFrame, itinerary: pl.DataFrame = None, output_path: str = "output/aggregate_report.html") -> str:
    """
    Generate an interactive HTML table showing the aggregate zone report.
    
    Args:
        aggregate_df: Polars DataFrame containing aggregate zone metrics
        output_path: Path to save the HTML file
        
    Returns:
        Path to the saved HTML file
    """
    logger.info(f"Generating aggregate report HTML table at {output_path}")
    
    # Ensure output directory exists
    output_dir = Path(output_path).parent
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Convert DataFrame to list of dictionaries for easier HTML generation
    data_rows = aggregate_df.to_dicts()
    
    # Generate HTML table rows
    table_rows_html = ""
    for row in data_rows:
        # Format numeric values for better display
        weekly_duration = f"{row['weekly_duration']:.2f}"
        utilization = f"{row['utilization']:.1f}%"
        total_pos_time = f"{row['total_pos_time']:.2f}"
        total_drive_time = f"{row['total_drive_time']:.2f}"
        
        table_rows_html += f"""
            <tr>
                <td>{row['zone_id']}</td>
                <td>{row['primary_pos_count']}</td>
                <td>{row['secondary_pos_count']}</td>
                <td>{weekly_duration}</td>
                <td>{utilization}</td>
                <td>{row['overutilized_days']}</td>
                <td>{row['underutilized_days']}</td>
                <td>{total_pos_time}</td>
                <td>{total_drive_time}</td>
            </tr>
        """
    
    # Generate summary statistics
    total_zones = len(data_rows)
    avg_weekly_duration = sum(row['weekly_duration'] for row in data_rows) / total_zones if total_zones > 0 else 0
    avg_utilization = sum(row['utilization'] for row in data_rows) / total_zones if total_zones > 0 else 0
    avg_pos_time = sum(row['total_pos_time'] for row in data_rows) / total_zones if total_zones > 0 else 0
    avg_drive_time = sum(row['total_drive_time'] for row in data_rows) / total_zones if total_zones > 0 else 0
    ...
    <div class="summary-stats">
        ...
        <div class="stats-grid">
            <div class="stat-item">
                <strong>Total Zones:</strong> {total_zones}
            </div>
            <div class="stat-item">
                <strong>Average Weekly Duration:</strong> {avg_weekly_duration:.1f} hrs
            </div>
            <div class="stat-item">
                <strong>Average Utilization:</strong> {avg_utilization:.1f}%
            </div>
            <div class="stat-item">
                <strong>Average POS Time:</strong> {avg_pos_time:.1f} hrs
            </div>
            <div class="stat-item">
                <strong>Average Drive Time:</strong> {avg_drive_time:.1f} hrs
            </div>
        </div>
    </div>
    """
    
    # Generate map section if itinerary data is provided
    map_section = ""
    if itinerary is not None:
        try:
            logger.info("Generating comprehensive zones map for aggregate report")
            map_html = create_comprehensive_zones_map(itinerary)
            map_section = f"""
                <div class="table-container mt-3">
                    <h3 class="mb-1">All Zones Map</h3>
            ...
            logger.error(f"Failed to generate zones map: {e}")
            map_section = """
                <div class="table-container mt-3">
                    <h3 class="mb-1">All Zones Map</h3>
                    <p class="text-muted">Map generation failed - see logs for details</p>
                </div>
            """
    
    # Create the complete HTML page using master styles
    full_html = f"""
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Route Optimization - Aggregate Report</title>
    ...
    <style>
        ...
        /* DataTables filter positioning - multiple selectors to catch different structures */
        #aggregate-table_filter,
        .dataTables_filter,
        div[id$="_filter"] {{
        ...
        #aggregate-table_filter label,
        .dataTables_filter label,
        div[id$="_filter"] label {{
            margin-left: 0 !important;
            padding-left: 0 !important;
        }}
        #aggregate-table_filter input,
...
<body>
    <div class="container">
        <div class="row">
            <div class="col-12">
                <h1 class="text-center mb-4">Route Optimization - Zone Aggregate Report</h1>
                
                {summary_html}
                
                <div class="table-container">
                    <h3 class="mb-3">Zone-Level Metrics</h3>
                    <div class="table-responsive" style="overflow-x: auto; max-width: 100%;">
                        <div style="max-width: 100%; overflow: hidden;">
                            <table id="aggregate-table" class="table table-striped table-hover" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Zone ID</th>
                                    <th>Primary Locations</th>
                                    <th>Secondary Locations</th>
                                    <th>Weekly Duration (hrs)</th>
                                    <th>Utilization (%)</th>
                                    <th>Overutilized Days</th>
                                    <th>Underutilized Days</th>
                                    <th>Total POS Time (hrs)</th>
                                    <th>Total Drive Time (hrs)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {table_rows_html}
    ...
    <script>
        $(document).ready(function() {{
            $('#aggregate-table').DataTable({{
                ...
                "columnDefs": [
                    {{ 
                        "targets": [1, 2, 5, 6], // Primary count, secondary count, over/under days
                        "className": "text-center"
                    }},
                    {{ 
                        "targets": [3, 4, 7, 8], // Duration, utilization, position time, drive time  
                        "className": "text-center"
                    }},
                    {{ "targets": 0, "width": "10%" }}, // Zone ID
                    {{ "targets": 1, "width": "8%" }}, // Primary Locations
                    {{ "targets": 2, "width": "8%" }}, // Secondary Locations  
                    {{ "targets": 3, "width": "12%" }}, // Weekly Duration
                    {{ "targets": 4, "width": "10%" }}, // Utilization
                    {{ "targets": 5, "width": "8%" }}, // Overutilized Days
                    {{ "targets": 6, "width": "8%" }}, // Underutilized Days
                    {{ "targets": 7, "width": "12%" }}, // Total POS Time
                    {{ "targets": 8, "width": "12%" }} // Total Drive Time
                ],
                "dom": 'Bfrtip',
                "buttons": [
...
"""
    ...
    logger.success(f"Aggregate report HTML saved to {output_path}")
    return output_path


if __name__ == "__main__":
    # Example usage and testing
    logger.info("Testing html_utils.create_aggregate_report_html() function")
    ...
    mock_data = [
        {
            'zone_id': 'zone_001',
            'primary_pos_count': 1,
            'secondary_pos_count': 12,
            'weekly_duration': 38.25,
            'utilization': 95.62,
            'overutilized_days': 1,
            'underutilized_days': 0,
            'total_pos_time': 34.0,
            'total_drive_time': 4.25
        },
        {
            'zone_id': 'zone_025',
            'primary_pos_count': 0,
            'secondary_pos_count': 10,
            'weekly_duration': 10.94,
            'utilization': 27.35,
            'overutilized_days': 0,
            'underutilized_days': 5,
            'total_pos_time': 10.0,
            'total_drive_time': 0.94
        }
    ]
    
    mock_df = pl.DataFrame(mock_data)
    logger.info("Mock aggregate data:")
    logger.info(mock_df)
    
    # Test HTML generation
    output_path = create_aggregate_report_html(mock_df, "output/test_aggregate_report.html")
    logger.info(f"Test HTML report generated: {output_path}")


# ==============================================================================
# Route Visualization Functions (moved from visualization.py)
# ==============================================================================
...
class RouteVisualizer:
    ...
    def __init__(self, locations_path: str = "data/subway_locations.jsonl"):
        ...
        self.day_colors = {
            1: '#FF0040',  # Vibrant Red
            2: '#00FF80',  # Vibrant Green
            3: '#0080FF',  # Vibrant Blue
            4: '#FF8000',  # Vibrant Orange
            5: '#8000FF'   # Vibrant Purple
        }
    ...
    def create_overview_map(self, results: Dict[str, Any]) -> go.Figure:
        ...
        fig.update_layout(
            mapbox=dict(
                style='carto-positron',  # Dark theme for better color contrast
...
def generate_itinerary_html(optimization_package) -> str:
    ...
    for row in itinerary_rows:
        if row['type'] == 'day_header':
            ...
            itinerary_html += f"""
                <tr class="day-header" style="background-color: #404040; color: #ffffff; font-weight: bold;">
                    <td>Day {row['day']}</td>
                    <td></td>
                    <td></td>
                    <td></td>
...
def create_geocoding_results_map(locations_data: List[Dict], output_path: str = "output/geocoding_results_map.html") -> str:
    ...
    summary_html = f"""
    <div style="margin: 20px; padding: 20px; background-color: #2a2a2a; border: 1px solid #404040; border-radius: 8px; color: #e0e0e0;">
...
def create_individual_zone_report(zone_itinerary: pl.DataFrame, zone_id: str, output_path: str) -> str:
    """
    Generate an HTML report for an individual zone.
    
    Args:
        zone_itinerary: DataFrame containing itinerary data for a single zone
    ...
    logger.info(f"Generating individual zone report for {zone_id}")
    
    # Ensure output directory exists
    output_dir = Path(output_path).parent
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Generate zone summary statistics
    zone_summary = generate_zone_summary(zone_itinerary, zone_id)
    
    # Generate zone map
    zone_map_html = create_individual_zone_map(zone_itinerary, zone_id)
    
    # Generate itinerary table from DataFrame
    itinerary_table_html = generate_itinerary_table_from_df(zone_itinerary, zone_id)
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Route Optimization - {zone_id.upper()}</title>
...
<body>
    ...
    logger.success(f"Individual zone report saved to {output_path}")
    return output_path


def generate_zone_summary(zone_itinerary: pl.DataFrame, zone_id: str) -> str:
    """Generate summary statistics for a single zone."""
    import yaml
    
    # Load config for utilization calculation
    try:
    ...
    total_days = zone_itinerary.height
    total_weekly_duration = zone_itinerary['duration'].sum() / 60.0  # Convert to hours
    ...
    utilization = (total_weekly_duration / target_weekly_hours) * 100 if target_weekly_hours > 0 else 0
    ...
    <div class="summary-stats">
        ...
        <div class="stats-grid">
            <div class="stat-item">
                <strong>Total Locations:</strong> {len(all_pos_ids)} ({primary_count} primary, {secondary_count} secondary)
            </div>
            <div class="stat-item">
                <strong>Weekly Duration:</strong> {total_weekly_duration:.1f} hrs
            </div>
            <div class="stat-item">
                <strong>Utilization:</strong> {utilization:.1f}%
            </div>
            <div class="stat-item">
                <strong>Total POS Time:</strong> {total_pos_time:.1f} hrs
            </div>
            <div class="stat-item">
                <strong>Total Drive Time:</strong> {total_drive_time:.1f} hrs
            </div>
        </div>
    </div>
    """
...
def get_gradient_opacity(stop_index, total_stops, min_opacity=0.3, max_opacity=1.0):
    """
    Generate an opacity value based on stop position.
    Creates a smooth transition from lighter opacity to full opacity.
    
    Args:
        stop_index: Current stop index (0-based)
        total_stops: Total number of stops
        min_opacity: Minimum opacity (0.3 = 30% opacity at start)
        max_opacity: Maximum opacity (1.0 = 100% opacity at end)
    
    Returns:
        Opacity value between min_opacity and max_opacity
    """
    if total_stops <= 1:
        # Single stop, use 70% opacity
        return 0.7
    else:
        # Calculate opacity: first stop is most transparent, last stop is most opaque (inverted)
        return min_opacity + (max_opacity - min_opacity) * (stop_index / (total_stops - 1))

def create_individual_zone_map(zone_itinerary: pl.DataFrame, zone_id: str) -> str:
    """Create a map for an individual zone showing detailed routes for each day with gradient colors."""
    try:
        import plotly.graph_objects as go
        ...
        try:
            ...
            logger.warning(f"Locations file not found: {locations_path}")
            return "<p>Map generation failed - locations file not found</p>"
        
        fig = go.Figure()
        
        # Define base colors for different days
        day_colors = [
            '#FF0040',  # Vibrant Red
        ...
        for row in zone_itinerary.iter_rows(named=True):
            ...
            if not route or not pos_locations:
                continue
            ...
            # Create gradient route segments and markers
            if num_stops <= 1:
                # Single stop - use base color with standard opacity
                legend_color = base_color
                
                # Add single marker
                if pos_locations and len(pos_locations[0]) >= 2:
                    lon, lat = pos_locations[0][0], pos_locations[0][1]
                ...
                legend_color = base_color  # Use base color for legend
                
                # First, create route segments between stops
                if route_lats and route_lons and len(pos_locations) > 1:
                    # Find the closest route point to each stop location
                    stop_indices = []
                    for pos_loc in pos_locations:
                        if len(pos_loc) >= 2:
                            stop_lon, stop_lat = pos_loc[0], pos_loc[1]
                            min_dist = float('inf')
                            closest_idx = 0
                    ...
                    for seg_idx in range(len(stop_indices) - 1):
                        # Get segment opacity (opacity for the starting stop of this segment)
                        segment_opacity = get_gradient_opacity(seg_idx, num_stops)
                        
                        # Get route points for this segment
                        start_idx = stop_indices[seg_idx]
                        end_idx = stop_indices[seg_idx + 1] + 1  # +1 to include the end point
                        ...
                        if start_idx < len(route_lats) and end_idx <= len(route_lats) and start_idx < end_idx:
                            ...
                            if segment_lats and segment_lons:
                                fig.add_trace(go.Scattermapbox(
                                    ...
                                    showlegend=False,  # Don't show individual segments in legend
                                    legendgroup=f'day_{day}'
                                ))
                
                # Add gradient markers for each stop
                for i, (pos_location, pos_id, pos_class) in enumerate(zip(pos_locations, pos_ids, pos_classes)):
                    if len(pos_location) >= 2:
                        lon, lat = pos_location[0], pos_location[1]
                        
                        # Get location details
                        location_info = locations.get(pos_id, {})
                        name = location_info.get('name', f'Location {pos_id}')
                        
                        # Calculate gradient opacity for this stop
                        # First stop gets minimum opacity, other stops use gradient based on their position
                        if i == 0:
                            # First stop - use very low opacity (even lower than gradient minimum)
                            stop_opacity = 0.2  # Fixed low opacity for first stop
                        else:
                            # Other stops - use gradient based on stop position (not segment position)
                            stop_opacity = get_gradient_opacity(i, num_stops)
                        
                        # Determine marker style based on location class
                        marker_size = 24 if pos_class == 'primary' else 20
                        
                        # Show legend only for first marker of this day
                        show_in_legend = (i == 0)
                        legend_name = f'Day {day}' if show_in_legend else f'Day {day} - {pos_class.title()}'
                        ...
                        trace_params = dict(
                            ...
                            opacity=stop_opacity,  # Use calculated opacity for all stops
                        ...
                        # Note: Removed legendgrouptitle to avoid nested legend entries
                        
                        fig.add_trace(go.Scattermapbox(**trace_params))
        
        if not all_lats or not all_lons:
            return "<p>No route data available for this zone</p>"
        ...
        return fig.to_html(include_plotlyjs=True, div_id="zone-map")
        
    except Exception as e:
        logger.error(f"Failed to create zone map for {zone_id}: {e}")
        return f"<p>Map generation failed: {e}</p>"


def generate_itinerary_table_from_df(zone_itinerary: pl.DataFrame, zone_id: str) -> str:
    """Generate HTML table for the daily itinerary from DataFrame."""
    # Load location data to get store names
    from . import io_utils
    try:
        all_locations_df = io_utils.extract(zone_ids=None, locations_path="./data/locations.jsonl")
        location_names = {row['pos_id']: row['name'] for row in all_locations_df.iter_rows(named=True)}
    except Exception as e:
        logger.warning(f"Could not load location names: {e}")
        location_names = {}
    ...
    for row in zone_itinerary.iter_rows(named=True):
        ...
        # Create route details with store names, schedule times, and drive times as separate lines
        route_details = []
        for i, pos_id in enumerate(pos_ids):
            # Get store name
            store_name = location_names.get(pos_id, f"Store {pos_id}")
            
            # Get arrival time from schedule (convert minutes to HH:MM format)
            # Schedule pattern: [arrive_loc1, leave_loc1, arrive_loc2, leave_loc2, ...]
            if i * 2 < len(schedule):
            ...
            # Add drive time line before this location (if not first location)
            if i > 0 and (i * 2 - 1) < len(schedule) and (i * 2) < len(schedule):
                # Drive time = arrive_current - leave_previous
                previous_departure = schedule[i * 2 - 1]
                current_arrival = schedule[i * 2]
                drive_minutes = current_arrival - previous_departure
                
                if drive_minutes > 0:
                    # Get departure time for drive line
                    dep_hours = int(previous_departure // 60)
                    dep_minutes = int(previous_departure % 60)
                    departure_time = f"{dep_hours:02d}:{dep_minutes:02d}"
                    
                    drive_hours = int(drive_minutes // 60)
                    drive_mins = int(drive_minutes % 60)
                    if drive_hours > 0:
                        drive_duration = f"{drive_hours}h {drive_mins}m"
                    else:
                        drive_duration = f"{drive_mins}m"
                    
                    route_details.append(f"&nbsp;&nbsp;&nbsp;driving {i} → {i+1} - {departure_time} ({drive_duration})")
            
            # Add the location line
            route_details.append(f"{i+1}. {store_name} - {arrival_time}")
        
        route_str = "<br>".join(route_details) if route_details else "No locations"
    ...
    <div class="table-responsive">
        <table id="itinerary-table" class="table table-striped table-hover">
