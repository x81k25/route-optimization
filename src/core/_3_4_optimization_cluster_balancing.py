...
def balance_cluster_workloads(
    ...
    :param itinerary: Complete itinerary DataFrame with primary and secondary assignments
    :param od_matrix: Distance matrix
    :param model_params: Model parameters
    :param balancer: Balancing algorithm to use
    :return: Balanced itinerary DataFrame
    """
    zone_id = itinerary.select("zone_id").unique().to_series().to_list()[0]

    logger.info(f"stage 3.4: cluster balancing - zone {zone_id}")
    logger.info(f"starting {balancer} balancing on secondary clusters only")

    # Separate primary and secondary itineraries
    primary_itinerary = itinerary.filter(pl.col("pos_class") == "primary")
    secondary_itinerary = itinerary.filter(pl.col("pos_class") == "secondary")

    if len(secondary_itinerary) == 0:
        logger.info("no secondary locations to balance")
        return itinerary.with_columns(balancer=pl.lit(balancer))

    # Get balancing parameters from model_params
    duration_threshold_min = model_params.get("duration_threshold_min", 60.0)
    max_iterations = model_params.get("max_iterations", 5)

    # Convert secondary itinerary to day assignments dict for balancing
    secondary_assignments = {}
    unique_days = secondary_itinerary.select("day").unique().to_series().to_list()

    for day in unique_days:
        day_locations = secondary_itinerary.filter(pl.col("day") == day).select("pos_id").to_series().to_list()
        secondary_assignments[day] = day_locations

    # Create location DataFrame for balancing functions
    # Extract coordinates from the route column (first coordinate in the route list)
    location_rows = []
    for row in secondary_itinerary.unique(subset=["pos_id"]).iter_rows(named=True):
        pos_id = row["pos_id"]
        route = row.get("route", [])
        if route and isinstance(route, list) and len(route) > 0:
            ...
            if isinstance(coord, list) and len(coord) >= 2:
                ...
                location_rows.append({"pos_id": pos_id, "latitude": lat, "longitude": lon})

    location_df = pl.DataFrame(location_rows)

    # Apply the selected balancing algorithm
    if balancer == "none":
        logger.info("balancer set to 'none' - skipping cluster balancing")
        balanced_assignments = secondary_assignments
    elif balancer == "greedy":
        balanced_assignments = apply_greedy_transfer_balancing(
            secondary_assignments, location_df, od_matrix, zone_id, duration_threshold_min, max_iterations
        )
    elif balancer == "local_search":
        balanced_assignments = apply_local_search_balancing(
            secondary_assignments, location_df, od_matrix, zone_id, duration_threshold_min, max_iterations
        )
    elif balancer == "simulated_annealing":
        balanced_assignments = apply_simulated_annealing_balancing(
            secondary_assignments, location_df, od_matrix, zone_id, duration_threshold_min
        )
    elif balancer == "min_max":
        balanced_assignments = apply_min_max_balancing(
            secondary_assignments, location_df, od_matrix, zone_id, duration_threshold_min, max_iterations
        )
    elif balancer == "network_flow":
        balanced_assignments = apply_network_flow_balancing(
            secondary_assignments, location_df, od_matrix, zone_id
        )
    else:
    ...
    # Convert balanced assignments back to itinerary format
    balanced_secondary_rows = []
    for day, location_ids in balanced_assignments.items():
        for pos_id in location_ids:
            # Get the original row for this location
            original_row = secondary_itinerary.filter(pl.col("pos_id") == pos_id).row(0, named=True)
            # Update the day assignment
            updated_row = {**original_row, "day": day}
            balanced_secondary_rows.append(updated_row)

    # Create balanced secondary DataFrame
    balanced_secondary = pl.DataFrame(balanced_secondary_rows)

    # Combine primary and balanced secondary itineraries
    balanced_itinerary = pl.concat([primary_itinerary, balanced_secondary])

    # Add balancer column and sort
    balanced_itinerary = balanced_itinerary.with_columns(
        balancer=pl.lit(balancer)
    ).sort(["zone_id", "day", "pos_class", "pos_id"])

    logger.success(f"{balancer} balancing completed for zone {zone_id}")

    return balanced_itinerary
